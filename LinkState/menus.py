from utils import print_cyan

# estructura sugerida por chat gpt


def menu():
    print_cyan("\nMENU GENERAL: Qué desea hacer? (Ingresar número de opción)")
    print_cyan("1. Registrarse")
    print_cyan("2. Iniciar sesión")
    print_cyan("3. Eliminar cuenta")
    print_cyan("4. Salir")


def user_menu():
    print_cyan("\nXMPP CHAT: Qué desea hacer? (Ingresar número de opción)")
    print_cyan("1. Mostrar todos los contactos y su estado")
    print_cyan("2. Descubrir vecinos")
    print_cyan("3. Agregar contacto")
    print_cyan("4. Mostrar detalles de vecinos")
    print_cyan("5. Comunicacion 1 a 1 con cualquier usuario/contacto")
    print_cyan("6. Salir")
